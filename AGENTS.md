...
- Use `/Users/philliphounslow/plasius/schema` (`@plasius/game-audio`) as the baseline template when creating new `@plasius/*` packages.
...
### Local Notes
This repo is `@plasius/game-audio`, portable command, bus, policy, asset, telemetry, and rollout contracts for Plasius game audio.
